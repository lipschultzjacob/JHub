...
// The main transactions page: view and categorize transactions. (Connecting,
// syncing and disconnecting banks live on the Settings screen.) This runs on the server (a "Server Component" -- see
...
// browser. Only the interactive pieces further down (the push button,
// category dropdowns) are separate "Client Components" that do
      ...
      <div className={card}>
        ...
        {items.length === 0 && (
          <p className={`text-sm ${bodyText65}`}>
            No bank connected yet. Connect one in{" "}
            <Link href="/settings" className="underline hover:text-accent">
              Settings
            </Link>{" "}
            to start pulling transactions.
          </p>
        )}
        {items.length > 0 && rows.length === 0 && (
          <p className={`text-sm ${bodyText65}`}>No transactions yet -- try syncing from Settings.</p>
        )}
      </div>
